...
class LeaveType(Base):
    ...
    code: Mapped[str] = mapped_column(String(16), unique=True, nullable=False, index=True)  # CL/SL/PL/PM
    name: Mapped[str] = mapped_column(String(60), nullable=False)
    unit: Mapped[LeaveUnit] = mapped_column(SAEnum(LeaveUnit, name="leave_unit_enum"), nullable=False)
...
class LeaveRequest(Base):
    ...
    requested_hours: Mapped[Optional[float]] = mapped_column(Numeric(5,2), nullable=True)  # for HOUR

    status: Mapped[LeaveStatus] = mapped_column(
        SAEnum(LeaveStatus, name="leave_status_enum"), nullable=False, default=LeaveStatus.PENDING
    )
